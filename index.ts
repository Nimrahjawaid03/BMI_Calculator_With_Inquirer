...
function calculateBMI(weight: number, heightFeet: number): { bmi: number, category: string } {
...
const main = async () => {
    const { weight, heightFeet } = await inquirer.prompt(questions) as { weight: number, heightFeet: number };
